...
import Sprite.GM5;
import Tools.B2WorldCreator5;
import Tools.WorldContactListener5;

public class PlayScreen5 implements Screen { // 보너스 스테이지
	/* 다이얼로그박스 */
	...
	private WorldContactListener5 world_contact;
	private World world;
	private Box2DDebugRenderer b2dr;
	private B2WorldCreator5 creator;

	private GM5 player;
	private SpriteBatch batch = new SpriteBatch();

	public PlayScreen5(MyGdxGame game, boolean is_first) {
		// 음악 재생 소스코드
		MusicPlayer.BackMusicStop();
		MusicPlayer.MainBackgMusic("Music/HiddenStage.mp3");
		...
		creator = new B2WorldCreator5(this);

		player = new GM5(this);

		world_contact = new WorldContactListener5();
	...
	public void update(float dt) {
		...
		for (Enemy5 enemy : creator.getGoombas())
			enemy.update(dt);
